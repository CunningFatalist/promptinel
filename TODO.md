...
## Command/Internal separation follow-ups (2026-02-27 refactor)

These items capture follow-up improvements after the cmd/internal separation refactor and should guide future agents.

### 1) Unify file discovery logic for scan and sanitize

This is needed because `internal/sanitize` and `internal/engine` still contain similar file collection and dedup logic.

- [ ] Introduce a shared package for file walking/collection (for example `internal/files`).
- [ ] Move common path deduplication, symlink handling, and regular-file checks into shared helpers.
- [ ] Make both scan and sanitize consume the shared implementation.
- [ ] Add regression tests proving both commands target the same files for equivalent include/exclude filters.

### 2) Reduce command flag boilerplate

This is needed because parse/build patterns are consistent but still manually duplicated across commands.

- [ ] Add shared helpers for common flag groups (config and include/exclude filters).
- [ ] Keep command-specific flags local while reusing shared parsing helpers.
- [ ] Ensure helper usage keeps `cmd/*` focused on orchestration only.
- [ ] Add tests that protect flag precedence and `Changed(...)` behavior.

### 3) Split report layer by domain responsibility

This is needed because `internal/report` now serves scan, baseline, and sanitize and can grow into a catch-all package.

- [ ] Split report rendering into domain-specific files/modules (scan, sanitize, baseline).
- [ ] Keep shared formatting utilities local and minimal.
- [ ] Preserve output compatibility for existing text output.
- [ ] Add focused tests per renderer to keep failures localized.

### 4) Clarify severity filtering semantics in internal scan flow

This is needed because `internal/scan` currently returns findings already filtered by `warn-on`, which can be ambiguous.

- [ ] Document that returned findings are policy-filtered, or introduce explicit fields for raw vs reportable findings.
- [ ] Decide whether baseline generation should continue to rely on filtered findings or support raw findings.
- [ ] Add tests for chosen semantics to prevent accidental behavior drift.
- [ ] Reflect the decision in `README.md` and architecture docs.

### 5) Tighten command test boundaries

This is needed because some command tests still validate behavior beyond pure command orchestration.

- [ ] Keep `cmd` tests focused on args/flags parsing, request mapping, and exit/error mapping.
- [ ] Move domain behavior assertions to `internal/*` package tests where possible.
- [ ] Remove duplicate behavior checks split across `cmd` and internal tests.
- [ ] Ensure coverage remains high for new internal logic.
