...
## Code review findings (from full code review on 2026-02-25)

- [ ] Prevent symlink write-through in `sanitize --apply` by using `Lstat`, skipping symlinks by default,
  or adding explicit opt-in link following.
  (`cmd/sanitize.go`, around file collection/stat/write flow)
- [ ] Make scan resilient to symlinked directories and broken links by filtering symlinks during discovery
  and treating unreadable targets as non-fatal skipped files.
  (`internal/engine/engine.go`, around collect/read/stat flow)
- [ ] Add a trusted-config mode (or `--no-config-discovery`) so untrusted workspaces cannot silently
  weaken scanning policy via auto-discovered `.promptinel.yaml`.
  (`internal/config/config.go`, config discovery path)
- [ ] Sanitize/escape control characters in report path/message rendering to prevent terminal/log injection
  in CI and interactive output.
  (`internal/report/report.go`, findings rendering output)
- [ ] Make `sanitize` resilient to symlinked directories and other non-regular files by filtering/skipping
  them during discovery (and surfacing skip diagnostics instead of hard-failing).
  (`cmd/sanitize.go`, around collect/read/stat flow)
- [ ] Raise command-layer coverage on critical CLI paths (`scan`, `sanitize`, `baseline`) by adding tests for
  error branches and flag/path edge cases.
  (`cmd/scan.go`, `cmd/sanitize.go`, `cmd/baseline.go`)
- [ ] Add a dependency security audit step (e.g., `govulncheck`) and a periodic outdated-module review so
  vulnerability and staleness checks do not rely on ad-hoc/manual runs.
  (`go.mod`, CI workflow/tooling)
